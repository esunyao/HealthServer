...
class NutriController(private val service: NutriService) {
    ...
    @GetMapping("/capture-sessions/drafts") fun captureDrafts(@CurrentUser user: AuthenticatedUser) = ApiResponse.success(service.captureDrafts(user))
    ...
    @PostMapping("/capture-sessions/{sessionId}/submit") @ResponseStatus(HttpStatus.ACCEPTED) fun submitCapture(@CurrentUser user: AuthenticatedUser, @PathVariable sessionId: UUID, @RequestBody @Valid request: CaptureSubmitRequest) = ApiResponse.success(service.submitCapture(user, sessionId, request), "餐食已入库并提交识别")
    ...
    @PatchMapping("/meals/{mealId}") fun patchMeal(@CurrentUser user: AuthenticatedUser, @PathVariable mealId: Long, @RequestBody @Valid request: MealMetadataPatchRequest) = ApiResponse.success(service.patchMeal(user, mealId, request))
