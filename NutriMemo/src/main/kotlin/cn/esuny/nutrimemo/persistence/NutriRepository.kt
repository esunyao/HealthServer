...
class NutriRepository(private val jdbc: JdbcTemplate) {
    ...
    fun expiredSessions(now: OffsetDateTime) = jdbc.query("SELECT * FROM nutri.meal_capture_sessions WHERE expires_at < ? AND status IN ('created','uploading','ready_for_analysis','analysing')", sessionMapper, now)
    fun expireSession(id: UUID) = jdbc.update("UPDATE nutri.meal_capture_sessions SET status='expired' WHERE capture_session_id=? AND status IN ('created','uploading','ready_for_analysis','analysing')", id)
    fun cancelSession(id: UUID, userId: UUID) = jdbc.update("UPDATE nutri.meal_capture_sessions SET status='cancelled' WHERE capture_session_id=? AND user_id=? AND status IN ('created','uploading','failed')", id, userId)
