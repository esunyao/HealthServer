...
class TaskCommandRepository(
    ...
    fun acceptCaptureReady(
        ...
        val release = jdbc.query(
            """
            SELECT tt.task_type_id, wr.release_id, wr.input_schema_version
            ...
            """.trimIndent(),
            emptyMap<String, Any>(),
        ) { rs, _ -> Triple(rs.getObject(1, UUID::class.java), rs.getObject(2, UUID::class.java), rs.getString(3)) }
            .singleOrNull()
        ...
        val manifest = objectMapper.createObjectNode()
            .put("capture_session_id", event.payload.captureSessionId)
        ...
        jdbc.update(
            ...
            mapOf(
                "taskId" to taskId,
                "taskTypeId" to release.first,
                "releaseId" to release.second,
                "eventId" to event.eventId,
                ...
                "traceId" to event.traceId,
                "schemaVersion" to release.third,
    ...
    fun claimNext(): TaskExecution? {
        val rows = jdbc.query(
            ...
            SELECT t.task_id, t.lock_version, t.subject_id, t.aggregate_type, t.aggregate_id, t.trace_id,
                   (t.context_manifest->>'capture_session_id')::bigint AS capture_session_id,
    ...
    fun fail(command: TaskExecution, failure: Throwable, category: FailureCategory, code: String) {
        ...
        node.set("payload", objectMapper.createObjectNode()
            ...
            .put("capture_session_id", command.captureSessionId)
